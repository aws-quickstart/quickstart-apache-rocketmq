...
The default deployment installs the web console on NameServer instances. However, as NameServer instances run in the private subnets, you cannot access the 8080 port of NameServer private IP addresses directly. 
...
--
. After configuring the SSH tunnel, set up a proxy manager in your browser. There are many proxy manager plug-ins available. These instructions use SwitchyOmega.
