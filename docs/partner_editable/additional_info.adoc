...
=== Logging into Apache RocketMQ NameServer and Broker server nodes
...
[start=3]
. Get the IP address for the node you want.
.. Log into the Amazon EC2 console.
. Select the To login to Nameserver or Broker Node from Bastion Host, and how to get an IP from Nameserver or Broker node, log in to ec2 console and click on the corresponding ec2  instance, as shown in the following
...
[start=4]
. Once you are signed into Nameserver or Broker Node, you can use Apache RocketMQ’s own command-line tools.


=== How to view Web Console from Apache RocketMQ using your browser

The default deployment installs the web console on each NameServer instance. However, as NameServer instances run on the private subnets, you cannot access the 8080 port of the NameServer private IP address directly through a browser. To do this, please follow the steps below to establish an SSH Tunnel to Bastion host and access The Nameserver Web Console via SSH Tunnel.
